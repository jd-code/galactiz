...
//---------- objets decentrés pour voir vis a vis de compound et companie ----------------

class TDTordu1 : public TDObj	// l'origine est bien au milieu
{
	TDString * pnums;
	vector<Vector3> vv;
    public:
	TDTordu1 (void)
	    {	vv.push_back (Vector3(-1.0, -1.0, 0.0));
		vv.push_back (Vector3( 1.0, -1.0, 0.0));
		vv.push_back (Vector3( 1.0,  1.0, 0.0));
		vv.push_back (Vector3(-1.0,  1.0, 0.0));
		pnums = new TDString ("012345678901234567890123456789", 0.12);
		pnums->setcolor (GLRGBA_YELLOW);
	    }
	void render (void)
	    {
		glMaterialfv (GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, (GLfloat *) GLRGBA_BLACK );
		...
		glLineWidth (1.0);
		glPushMatrix();
glTranslatef (0.0, 0.0, -0.1);	// légèrement en avant
		glBegin (GL_POLYGON);
		render_circle ();
		glPopMatrix();
		glMaterialfv (GL_FRONT_AND_BACK, GL_EMISSION,            (GLfloat *) GLRGBA_TRANSPBLACK );
	...
	virtual void proj_size (Vector3 &offset, Vector3 &size)
	    {	::proj_size (offset, size, vv, Vector3 (0,0,0), Vector3 (scale, scale, scale), rm);
	    }
	virtual GLfloat diameter (void)
	    {	return scale;
	    }
	virtual void gotclicked (SDL_Event const &event)
	    {	Vector3 offset, size;
		proj_size (offset, size);
		bzouzerr << "off=" << offset << "  size=" << size << endl ;
	    }
};

class TDTordu2 : public TDObj	// l'origine est un peu decalée à gauche
{
	TDString * pnums;
	vector<Vector3> vv;
    public:
	TDTordu2 (void)
	    {	
		vv.push_back (Vector3(-0.5, -1.0, 0.0));
		vv.push_back (Vector3( 1.5, -1.0, 0.0));
		vv.push_back (Vector3( 1.5,  1.0, 0.0));
		vv.push_back (Vector3(-0.5,  1.0, 0.0));
		pnums = new TDString ("012345678901234567890123456789", 0.12);
		pnums->setcolor (GLRGBA_YELLOW);
...
class TDTordu3 : public TDObj	// l'origine est un peu decalée à gauche
{
	TDString * pnums;
	vector<Vector3> vv;
    public:
	TDTordu3 (void)
	    {	
		vv.push_back (Vector3(-0.5, -0.7, 0.0));
		vv.push_back (Vector3( 1.5, -0.7, 0.0));
		vv.push_back (Vector3( 1.5,  1.3, 0.0));
		vv.push_back (Vector3(-0.5,  1.3, 0.0));
		pnums = new TDString ("012345678901234567890123456789", 0.12);
		pnums->setcolor (GLRGBA_YELLOW);
	    }
	void render (void)
...
//---------- fin objets decentrés pour voir vis a vis de compound et companie ------------
